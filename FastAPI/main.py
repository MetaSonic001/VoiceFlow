...
async def get_onboarding_status(agent_id: int | None = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    Return onboarding status for the current user. If agent_id is provided, return status for that agent.
    Response shape:
    { status: 'not_started'|'in_progress'|'completed', agent_active: bool, agent: { id, name } | None }
    """
    # if agent_id provided, try to fetch that agent
    agent = None
    if agent_id:
        agent = db.query(AIAgent).filter(AIAgent.id == agent_id, AIAgent.company_id == current_user.company_id).first()

    # if no agent specified, attempt to find any agent for the company
    if not agent:
        agent = db.query(AIAgent).filter(AIAgent.company_id == current_user.company_id).order_by(AIAgent.created_at.desc()).first()

    if not agent:
        return {"status": "not_started", "agent_active": False, "agent": None}
    ...
    return {"status": status, "agent_active": bool(agent.is_active), "agent": {"id": agent.id, "name": agent.name}}
...
async def save_onboarding_progress(payload: OnboardingProgressIn, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Upsert progress for the current user/company
    prog = db.query(OnboardingProgress).filter(OnboardingProgress.user_id == current_user.id).first()
    if not prog:
        prog = OnboardingProgress(user_id=current_user.id, company_id=current_user.company_id)
        db.add(prog)

    if payload.agent_id is not None:
        prog.agent_id = payload.agent_id
    if payload.current_step is not None:
        prog.current_step = payload.current_step
    if payload.data is not None:
        # merge existing data
        existing = prog.data or {}
    ...
    return {"success": True, "progress_id": prog.id, "agent_id": prog.agent_id, "current_step": prog.current_step, "data": prog.data}


@app.get('/onboarding/progress')
async def get_onboarding_progress(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    prog = db.query(OnboardingProgress).filter(OnboardingProgress.user_id == current_user.id).first()
    if not prog:
        return {"exists": False}
    return {"exists": True, "progress_id": prog.id, "agent_id": prog.agent_id, "current_step": prog.current_step, "data": prog.data}


@app.delete('/onboarding/progress')
async def delete_onboarding_progress(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    prog = db.query(OnboardingProgress).filter(OnboardingProgress.user_id == current_user.id).first()
    if prog:
        db.delete(prog)
        db.commit()

...
This manager prefers to use the backend application's SQLAlchemy-managed
database (via backend_adapter). If the backend is not available at runtime
the manager falls back to writing pending JSON files to disk so ingestion can
continue and be flushed later.
"""
...
class DatabaseManager:
    def __init__(self):
        self._pending_dir = pathlib.Path(os.getenv('PENDING_DOCS_DIR', './pending_documents'))
        try:
